...
static WFWSSERVER *server_remainhead = NULL;
...
static WFWS *client_remainhead = NULL;
...
int Send_Ws_Data (WFWS *wfws, unsigned char *data, unsigned long datasize, WS_DATA_TYPE ws_data_type) {
    if (datasize < 126) {
        ...
        if (Wf_Nio_Write_fd (wfws->asyncio, senddata, datasize + 2)) {
            printf("in %s, at %d\n", __FILE__, __LINE__);
            Wf_Nio_Error_Ws(wfws->asyncio, wfws->fd, wfws, 0);
            return -1;
        }
        ...
        if (Wf_Nio_Write_fd (wfws->asyncio, senddata, datasize + 4)) {
            printf("in %s, at %d\n", __FILE__, __LINE__);
            Wf_Nio_Error_Ws(wfws->asyncio, wfws->fd, wfws, 0);
            return -2;
        }
